...
##### My custom functions #####
function Invoke-PAFishTest {
    # Get the user's Documents folder
    $userDocs = [Environment]::GetFolderPath("MyDocuments")
    $pafishDir = Join-Path $userDocs "PAFish"

    # Ensure the PAFish directory exists
    if (-not (Test-Path $pafishDir)) {
        New-Item -Path $pafishDir -ItemType Directory | Out-Null
    }

    # Full path to pafish.exe in Documents/PAFish
    $pafishExe = Join-Path $pafishDir "pafish.exe"

    # PAFish download URL
    $pafishUrl = "https://github.com/a0rtega/pafish/releases/download/v0.6/pafish.exe"

    Write-Host "[+] Downloading PAFish to $pafishDir..."
    try {
        Invoke-WebRequest -Uri $pafishUrl -OutFile $pafishExe -UseBasicParsing
        Write-Host "[+] PAFish downloaded successfully to $pafishExe"
    } catch {
        Write-Error "Failed to download PAFish: $_"
        return
    }

    # Execute pafish.exe
    Write-Host "[+] Executing PAFish..."
    try {
        Start-Process -FilePath $pafishExe -Wait
        Write-Host "[+] PAFish execution completed"
    } catch {
        Write-Error "Failed to execute PAFish: $_"
        return
    }

    # Check for hi_vmware presence
    $hiFile = Join-Path $pafishDir "hi_vmware"
    if (Test-Path $hiFile) {
        Write-Error "hi_vmware detected! This indicates the environment is being detected by PAFish."
    } else {
        Write-Host "[+] hi_vmware not found. Environment appears stealthy."
    }
}

function Clean-PAFishArtifacts {
    # Get the user's Desktop folder
    $desktopPath = [Environment]::GetFolderPath("Desktop")

    # Remove all hi_* files except hi_vmware
    Get-ChildItem -Path $desktopPath -Filter "hi_*" -File | ForEach-Object {
        if ($_.Name -ne "hi_vmware") {
            try {
                Remove-Item -Path $_.FullName -Force
                Write-Host "[+] Removed file: $($_.Name)"
            } catch {
                Write-Warning "Failed to remove file $($_.Name): $_"
            }
        }
    }
}
...
function Install-CustomPackages {
    ...
    # ---------- OOSU10 (.reg) ----------
    try {
        $regFile = Join-Path $PSScriptRoot "misc\O-OSU10\OOSU10.reg"
        if (-not (Test-Path $regFile)) { throw "Registry file not found: $regFile" }

        Write-Host "[+] Importing registry file: $regFile"
        Start-Process "reg.exe" -ArgumentList @("import", "`"$regFile`"") -Verb RunAs -Wait
        Write-Host "[+] Registry file imported successfully.`n"
    } catch {
    ...
    try {
        $pkgName = "PMAT-labs"
        $pkgPath = Join-Path $PSScriptRoot "samples\PMAT-labs"
        if (-not (Test-Path $pkgPath)) { throw "Folder not found: $pkgPath" }

        Write-Host "[+] Moving $pkgName folder to Desktop..."
        $desktopFolder = Join-Path $env:USERPROFILE "Desktop\$pkgName"
        if (Test-Path $desktopFolder) {
            Write-Host "[!] Desktop folder already exists, merging contents..."
        }
        Move-Item -Path $pkgPath -Destination $desktopFolder -Force
        Write-Host "[+] $pkgName moved to Desktop successfully.`n"
    } catch {
        Write-LogException $_
    }
    ...
    try {
        $pkgName = "theZoo"
        $pkgPath = Join-Path $PSScriptRoot "samples\TheZoo"
        if (-not (Test-Path $pkgPath)) { throw "Folder not found: $pkgPath" }
...
Invoke-PAFishTest
Clean-PAFishArtifacts
Install-CustomPackages
...
Save-FileFromUrl -fileSource 'https://raw.githubusercontent.com/Ena-Shepherd/Shadow-VM/refs/heads/main/img/fish.png' -fileDestination $backgroundImage
